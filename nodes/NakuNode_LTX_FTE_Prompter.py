...
**时长设定**：
- **时长参数**：根据用户输入的秒数进行设置，范围 0-20 秒，默认值 5 秒
...
#### 4.4 运镜轨迹设计
**核心原则**：
1. 用户给出运镜路径 → 严格按用户路径写"起点→过程→终点"，终点不得改写
2. 用户未指定运镜 → 按决策树自动选择
...
### 示例 1：纯文本输入（带对话）
**输入**：一个人说"看那边！"，然后指向远方，镜头拉远。**视频时长**：5
...
### 示例 2：图像 + 文字输入（动作场景）
**输入**：图像中女子站在雨中，文字：她在雨中奔跑，背景有爆炸。**视频时长**：5
...
### 示例 4：纯文本输入（高能动作 + 多角色）
**输入**：两个超级英雄在空中飞行战斗，激光武器对射，背景是燃烧的城市。**视频时长**：5
...
### 示例 5：纯文本输入（温馨对话）
**输入**：母女在海边看日落，母亲说"你长大了"，女儿微笑。**视频时长**：7
...
### 示例 6：纯文本输入（动作 + 解说）
**输入**：赛车解说：赛车在赛道上呼啸而过，弯道处轮胎剧烈摩擦地面冒出白烟，引擎声震耳欲聋。**视频时长**：3
...
**输出**：
{
  ...
  "sync_notes": 轮胎冒烟与白烟视觉同步，过弯与音乐高潮同步
    ...
    def generate_ltx_prompt(self, First_Frame_Image, Last_Frame_Image, Video_Duration, User_Description, api_string, random_seed,
        ...
        try:
            ...
            print(f"[NakuNode LTX_FTE_Prompter] API Provider: {api_provider}")
            print(f"[NakuNode LTX_FTE_Prompter] SiliconFlow API Key: {'已设置' if sf_key else '未设置'}")
            print(f"[NakuNode LTX_FTE_Prompter] Custom API Key: {'已设置' if c_key else '未设置'}")
            print(f"[NakuNode LTX_FTE_Prompter] Custom API URL: {c_url}")

            print(f"\n[NakuNode LTX_FTE_Prompter] {'='*60}")
            print(f"[NakuNode LTX_FTE_Prompter] Starting request...")
            ...
            if use_stream:
                ...
                for attempt in range(max_retries):
                    try:
                        print(f"[NakuNode LTX_FTE_Prompter] Sending request (attempt {attempt + 1}/{max_retries})...")
                        ...
                        if response.status_code == 200:
                            ...
                            if response.status_code >= 500 and attempt < max_retries - 1:
                                print(f"[NakuNode LTX_FTE_Prompter] Server error, retrying in {retry_delay} seconds...")
                                time.sleep(retry_delay)
                                continue
                            return (f"Error: {error_msg}. Original: {full_description}", f"Error: {error_msg}")
                    except requests.exceptions.ConnectionError as e:
                        print(f"[NakuNode LTX_FTE_Prompter] Connection error (attempt {attempt + 1}/{max_retries}): {str(e)}")
                        if attempt < max_retries - 1:
                            print(f"[NakuNode LTX_FTE_Prompter] Retrying in {retry_delay} seconds...")
                            time.sleep(retry_delay)
                            retry_delay *= 2  # 指数退避
                        else:
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "NakuNode_LTX_FTE_Prompter": "NakuNode LTX FTE Prompter"
}
