...
class NakuNode_LTX_FTE_Prompter:
    """
    LTX视频提示词生成器 - 基于两张图片和描述生成LTX Video专用提示词 (FTE = First and Tail End)
    """
    ...
    def call_llm_api(self, provider, api_key, prompt, first_image_base64, tail_image_base64, siliconflow_model_choice="QWENVL"):
        # LTX Video 专用系统提示词
        system_prompt = """* Role
...
> 首尾帧画面补全：用户提供两张图片，请根据用户的简单描述，结合图片自行补全图片与图片之间的动态变化。要求动作连贯，剧情合理。不要叠化转场过渡，要符合逻辑。
...
[A detailed, descriptive paragraph in English. Starts with the main subject and action. Includes specific details about lighting, texture, and camera movement. Flowing naturally without breaking into comma-separated tags.]"""
        ...
        if provider == "智谱AI":
            ...
            messages = [
                ...
                {"role": "user", "content": [
                    {"type": "text", "text": f"根据以下用户描述和首尾帧图片，生成LTX Video专用提示词：{prompt}"},
    ...
    def generate_ltx_prompt(self, 首帧图片, 尾帧图片, 用户描述, AI服务商, API_KEY, 随机种子, 硅基流动模型选择="QWENVL", **kwargs):
        # 设置随机种子
        if 随机种子 == -1:
        ...
        try:
            print(f"[NakuNode LTX_FTE_Prompter] Generating LTX prompt with {AI服务商}...")
            # 如果是硅基流动服务商，传递模型选择
            if AI服务商 == "硅基流动":
                optimized_response = self.call_llm_api(AI服务商, API_KEY, full_prompt, first_image_base64, tail_image_base64, 硅基流动模型选择)
            else:
                optimized_response = self.call_llm_api(AI服务商, API_KEY, full_prompt, first_image_base64, tail_image_base64)
