...
6、特別注意：确保镜头与镜头之间的衔接流畅，不能出现例如「手持跟拍轨道推进」这种不合理的运镜方式，因为手持跟拍是有抖动的，轨道推进是平滑的。最终输出一段中文的自然语言，不能换行，不能在段头和段尾增加意义不明的符号，提示词以：中文提示词/... 这样的格式输出。例如：中文提示词/一位身穿红色西装外套的女性..."""
        ...
        if provider == "内置LLM1":
            endpoint = "https://api.algion.dev/v1/chat/completions"
            ...
            response = requests.post(endpoint, headers=headers, data=json.dumps(payload), timeout=180, verify=False)
            response.raise_for_status()
            result = response.json()
            return result['choices'][0]['message']['content']

        elif provider == "内置LLM2":
